...
void display(ICESTATE s, int round, char* step){
    printf("round %d step %s\n", round, step);
    for (int i = 0; i < 4;i ++ ){
        for(int j = 0; j < 5;j ++ ) {
            printf("%#llX ", s[i][j]);
        }
...
/* Reads 'blocklen' bytes of plaintext data (message or authenticated data)
   from 'source' and absorbs it into the ICEPOLE state S. 
   If dest is not NULL, then it also produces corresponding ciphertext
   block and writes it at 'dest' moving the pointer forward by 'blocklen'. 
   'frameBit' becomes part of the block padding.  */
...
)
{
	...
	unsigned int qlen; 
	unsigned int i = 0;
	assert(blocklen <= ICEPOLEDATABLOCKLEN);
	    
    /* absorb the data block (and produce the ciphertext) */
	do {
...
/* Reads 'blocklen' bytes of encrypted data (ciphertext, encrypted smn)
   from 'source' and absorbs it into the ICEPOLE state S. 
   If dest is not NULL, then it also produces corresponding plaintext
   block and writes it at 'dest' moving the pointer forward by 'blocklen'. 
   'frameBit' becomes part of the block padding.  */
...
/* Given the ICEPOLE state S generates the authentication tag and stores
 * in memory pointed by tag. */
void generateTag(
    ICESTATE S,
    unsigned char * tag
    )
